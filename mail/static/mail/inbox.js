...
function load_mailbox(mailbox) {
  ...
  .then((emails) => {
    emails.forEach((element) => {
      ...
      Subject: ${element.subject} | Recipient(s): ${sender_recipients} | Time: ${element.timestamp}
      ${element.body.slice(0,999)} </div>`;
...
function show_mail(id, mailbox) {
  ...
  .then((email) => {
    ...
    Subject: ${email.subject}
    Time: ${email.timestamp}
    <br><br> Content: ${email.body}
    </div>`;

    document.querySelector('#emails-view').appendChild(item);
    if (mailbox == 'sent') return;
    let archiveStatus = document.createElement("btn");
    archiveStatus.classNmae = 'btn btn-primary btn-sm';
    archiveStatus.addEventListener('click', function () {
      toggle_archiveStatus(id, email.archived);
      if (archiveStatus.innerText == 'Archive') archiveStatus.innerText = 'Unarchive';
      else {
        archiveStatus.innerText = 'Archive';
      }
    });

    if (!email.archived) archiveStatus.textContent = 'Archive';
    else {
      archiveStatus.textContent = 'Unarchive';
      document.querySelector('#emails-view').appendChild(archiveStatus);
      
      let reply = document.createElement('btn');
      reply.className = 'btn btn-primary btn-sm';
      reply.textContent = 'Reply email';
  ...
  function toggle_archiveStatus(id, state) {
    fetch{`/emails/${id}`, {
      method: "PUT",
      body: JSON.stringify({
        archived: !state,
  ...
  function reply_email(sender, subject, body, timestamp) {
    ...
    preloadedContent = `Time: ${timestamp} Sender: ${sender} \n ${body}\n`;
    
    document.querySelector('#compose-body').value = preloadedContent;
  }
*/
